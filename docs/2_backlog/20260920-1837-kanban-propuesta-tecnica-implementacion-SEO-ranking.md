...
| **BZ-105** | Incorporar en admin control accesible de promedio/cantidad, guardado y errores sin crecer el editor original ni borrar rating al editar otros campos. | BZ-104 | **En revisión** · isla separada y control adaptable; prueba manual autenticada pendiente |
| **BZ-106** | Crear un único componente SSR de estrellas para tarjeta y ficha; propagar a Home, catálogo, búsqueda y carrusel, ocultando estado vacío. | BZ-103 | **En revisión** · `Calificacion.astro`; prueba visual con producto valorado pendiente |
...
| **BZ-111** | Gates de cierre: typecheck, Node, Workers compatible, SDD, build, inspección SSR, accesibilidad, sesión/RLS y sondas de producción. | BZ-100 a BZ-109 | **En revisión** · 244 pruebas Node, build y SDD verdes; CI y humo del nuevo commit por confirmar; navegador autenticado y RLS real pendientes |
...
1. RED → GREEN verificado para las reglas nuevas. Tras SPEC-008: `npm run typecheck`, `npm run test:node` (244/244), `npm run build` y `npm run sdd:trace` pasan. Los avisos de deuda de otras SPEC en borrador son previos a esta entrega.
